...
class ProjectManager:
    ...
    def __init__(self):
        ...
        self.output_dir = project_config.get('defaults', {}).get('output_directory') or os.getenv('OUTPUT_DIR', './flutter_projects')
        self.flutter_sdk_path = project_config.get('flutter', {}).get('sdk_path') or os.getenv('FLUTTER_SDK_PATH', 'flutter')
    ...
    def _create_basic_structure(self, project_name: str) -> str:
        ...
        # Get file templates from config
        templates_config = self.config.get_templates_config()
        content_config = self.config.get_content_config()
        
        # Get pubspec template from config
        pubspec_template = templates_config.get('pubspec_template', {})
        environment_config = pubspec_template.get('environment', {})
        
        # Create basic pubspec.yaml with configurable values
        app_title = content_config.get('file_templates', {}).get('readme_title', 'Flutter Application')
        app_description = content_config.get('file_templates', {}).get('readme_description', 'A Flutter application generated by FlutterSwarm.')
...
environment:
  sdk: '{environment_config.get("sdk", ">=3.0.0 <4.0.0")}'
  flutter: "{environment_config.get("flutter", ">=3.0.0")}"

dependencies:
  flutter:
    sdk: flutter
  cupertino_icons: {pubspec_template.get("dependencies", {}).get("cupertino_icons", "^1.0.2")}

dev_dependencies:
  flutter_test:
    sdk: flutter
  flutter_lints: {pubspec_template.get("dev_dependencies", {}).get("flutter_lints", "^3.0.0")}
...
"""
        ...
        # Get default app title from config
        default_app_title = content_config.get('file_templates', {}).get('default_app_title', 'Flutter Demo')
        
        # Create basic main.dart with configurable title
...
class MyApp extends StatelessWidget {{
  ...
  Widget build(BuildContext context) {{
    return MaterialApp(
      ...
      home: const MyHomePage(title: '{default_app_title} Home Page'),
    );
  }}
}}
